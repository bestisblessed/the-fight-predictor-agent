...
- `app.py`: Flask app exposing `GET/POST /x/webhook` and `GET /healthz`. It verifies X CRC/signatures, writes webhook payloads to `logs/events_inbox.jsonl`, and disables in-process background work on PythonAnywhere.
- `pythonanywhere_wsgi.py`: PythonAnywhere WSGI entrypoint. This imports the Flask app as `application`.
- `pythonanywhere_worker.py`: Always-on task entrypoint for PythonAnywhere. It scans `logs/events_inbox.jsonl`, processes unhandled mentions, calls OpenAI, posts X replies, and records success/failure state.
...
- `settings.py`: Central configuration loader and validation. Reads `OPTIMIZED-PYTHONANYWHERE/.env`, validates `PUBLIC_BASE_URL`, and defines paths for data and logs, with the old state path retained only for migration.
...
- `storage.py`: JSON/JSONL file-backed persistence helpers for inbox logs, processed IDs, reply logs, failure logs, checkpoints, and webhook config.
...
- `logs/.gitkeep`: Keeps the logs directory in git. Runtime JSON/JSONL files in this directory are generated on the server.
...
4. `POST /x/webhook` verifies `x-twitter-webhooks-signature`, parses the JSON payload, appends it to `logs/events_inbox.jsonl`, and returns `200` quickly.
5. The PythonAnywhere Always-on task runs `pythonanywhere_worker.py` separately from the web request process.
6. The worker loads `OPTIMIZED-PYTHONANYWHERE/.env`, validates runtime settings, loads the local CSV datasets, and repeatedly scans `logs/events_inbox.jsonl`.
...
12. Successful replies are written to `logs/replies.jsonl`, and their dedupe keys are written to `logs/processed_event_ids.jsonl` with reason `replied`.
13. Failed jobs are written to `logs/failed_jobs.jsonl` with phase and error details. Retryable failures can be reprocessed with `python admin.py retry-failed`.
...
15. PythonAnywhere handles HTTPS/WSGI through the Web tab, and the Always-on task handles background processing.
