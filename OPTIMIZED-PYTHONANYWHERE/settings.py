...
class Config:
    ...
    def require_fighter_career_source(self) -> None:
        ...
        if fighter_dir.is_dir() and any(fighter_dir.glob("*.csv")):
            return
        if (self.data_dir / "fighters.zip").is_file():
            return
        raise RuntimeError(
            f"Missing fighter career data in {self.data_dir}: expected fighters/*.csv or fighters.zip"
        )
